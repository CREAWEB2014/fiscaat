 ...
 *
 * @uses fiscaat_get_version() To get the Fiscaat version
 */
function fiscaat_version() {
	echo fiscaat_get_version();
}
	/**
 ...
 * @uses fiscaat_get_version() To get the Fiscaat version
 */
function fiscaat_db_version() {
	echo fiscaat_get_db_version();
}
	/**
	 ...
	 * @return string The Fiscaat version
	 */
	function fiscaat_get_db_version() {
 ...
 */
function fiscaat_db_version_raw() {
	echo fiscaat_get_db_version_raw();
}
	/**
	 ...
	 */
	function fiscaat_get_db_version_raw() {
		return get_option( '_fiscaat_db_version', '' );
	}
 ...
 */
function fiscaat_update_record_id( $post_id, $record_id ) {

	// Allow the record ID to be updated 'just in time' before save
	$record_id = apply_filters( 'fiscaat_update_record_id', $record_id, $post_id );

	// Update the post meta record ID
	update_post_meta( $post_id, '_fiscaat_record_id', (int) $record_id );
}
 ...
 */
function fiscaat_update_account_id( $post_id, $account_id ) {

	// Allow the account ID to be updated 'just in time' before save
	$account_id = apply_filters( 'fiscaat_update_account_id', $account_id, $post_id );

	// Update the post meta account ID
	update_post_meta( $post_id, '_fiscaat_account_id', (int) $account_id );
}
 ...
 */
function fiscaat_update_year_id( $post_id, $year_id ) {

	// Allow the year ID to be updated 'just in time' before save
	$year_id = apply_filters( 'fiscaat_update_year_id', $year_id, $post_id );

	// Update the post meta year ID
	update_post_meta( $post_id, '_fiscaat_year_id', (int) $year_id );
}
 ...
 */
function fiscaat_add_error( $code = '', $message = '', $data = '' ) {
 ...
 */
function fiscaat_has_errors() {
	$has_errors = fiscaat()->errors->get_error_codes() ? true : false;

	return apply_filters( 'fiscaat_has_errors', $has_errors, fiscaat()->errors );
}
 ...
 * @return string
 */
function fiscaat_get_public_status_id() {
 ...
 * @return string
 */
function fiscaat_get_approved_status_id() {
	return fiscaat()->approved_status_id;
}

/**
 * Return the disapproved post status ID
 *
 * @return string
 */
function fiscaat_get_disapproved_status_id() {
	return fiscaat()->disapproved_status_id;
}
 ...
 * @return string
 */
function fiscaat_get_closed_status_id() {
 ...
 * @return string
 */
function fiscaat_get_edit_rewrite_id() {
 ...
 */
function fiscaat_delete_rewrite_rules() {
