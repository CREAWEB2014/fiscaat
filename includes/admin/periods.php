...
class Fiscaat_Periods_Admin {
	 ...
	 */
	public function row_actions( $actions, $period ) {
		if ( $this->bail() ) 
			return $actions;
		...
		if ( current_user_can( 'delete_period', $period->ID ) && ( fct_is_period_closed() || ! fct_period_has_records() ) ) {
			if ( fct_get_trash_status_id() == $period->post_status ) {
				...
				$actions['untrash'] = "<a title='" . esc_attr__( 'Restore this item from the Trash', 'fiscaat' ) . "' href='" . add_query_arg( array( '_wp_http_referer' => add_query_arg( array( 'post_type' => fct_get_period_post_type() ), admin_url( 'edit.php' ) ) ), wp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $period->ID ) ), 'untrash-' . $period->post_type . '_' . $period->ID ) ) . "'>" . __( 'Restore', 'fiscaat' ) . "</a>";
			} elseif ( EMPTY_TRASH_DAYS ) {
				$actions['trash'] = "<a class='submitdelete' title='" . esc_attr__( 'Move this item to the Trash', 'fiscaat' ) . "' href='" . add_query_arg( array( '_wp_http_referer' => add_query_arg( array( 'post_type' => fct_get_period_post_type() ), admin_url( 'edit.php' ) ) ), get_delete_post_link( $period->ID ) ) . "'>" . __( 'Trash', 'fiscaat' ) . "</a>";
			}

			if ( fct_get_trash_status_id() == $period->post_status || ! EMPTY_TRASH_DAYS ) {
				$actions['delete'] = "<a class='submitdelete' title='" . esc_attr__( 'Delete this item permanently', 'fiscaat' ) . "' href='" . add_query_arg( array( '_wp_http_referer' => add_query_arg( array( 'post_type' => fct_get_period_post_type() ), admin_url( 'edit.php' ) ) ), get_delete_post_link( $period->ID, '', true ) ) . "'>" . __( 'Delete Permanently', 'fiscaat' ) . "</a>";
			}
		}
