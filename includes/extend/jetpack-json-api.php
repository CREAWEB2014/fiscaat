...
class Fiscaat_Jetpack_REST_API {
	...
	private function __construct() {
		add_filter( 'rest_api_allowed_post_types', array( $this, 'allow_fct_post_types' ) );
		add_filter( 'fct_map_meta_caps', array( $this, 'adjust_meta_caps' ), 10, 4 );
		add_filter( 'rest_api_allowed_public_metadata', array( $this, 'allow_fct_public_metadata' ) );
	}

	function allow_fct_post_types( $allowed_post_types ) {
	...
	function allow_fct_public_metadata( $allowed_meta_keys ) {
		...
		$allowed_meta_keys[] = '_fct_total_account_count';
		$allowed_meta_keys[] = '_fct_account_count_hidden';
		$allowed_meta_keys[] = '_fct_last_account_id';
		$allowed_meta_keys[] = '_fct_last_record_id';
		$allowed_meta_keys[] = '_fct_last_active_time';
		$allowed_meta_keys[] = '_fct_last_active_id';
		$allowed_meta_keys[] = '_fct_voice_count';
		$allowed_meta_keys[] = '_fct_record_count_hidden';
	...
	function adjust_meta_caps( $caps, $cap, $user_id, $args ) {
		...
		if ( ! in_array( $cap, array( 'edit_post_meta', 'delete_post_meta', 'add_post_meta' ) ) || empty( $args[1] ) || false === strpos( $args[1], '_fct_' ) )
			return $caps;
